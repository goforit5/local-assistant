...
def get_pipeline() -> DocumentProcessingPipeline:
    ...
    if not openai_provider:
        # Create provider if not in app_state
        api_key = os.getenv("OPENAI_API_KEY", "")
        openai_provider = OpenAIProvider(ProviderConfig(api_key=api_key))
...
async def upload_document(
    ...
    try:
        ...
        links = LinksInfo(
            timeline=f"/api/interactions/timeline?entity_id={result.document_id}",
            vendor=f"/api/vendors/{result.vendor_id}" if result.vendor_id else None,
            download=f"/api/documents/{result.document_id}/download",
        )
